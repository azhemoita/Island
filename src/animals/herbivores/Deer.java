...
public class Deer extends Herbivore implements Livable {
    public static final int PROBABILITY_EATS_PLANT = 100;
    private Cell currentCell;
    private final AtomicReference<Double> currentWeight = new AtomicReference<>(Data.DEER.getWeight());
    ...
    @Override
    public void eat() {
        System.out.println("Deer at (" + currentCell.getCoordinate().getX()
        ...
        while (currentWeight.get() < Data.DEER.getWeight()) {
            if (plants.isEmpty()) {
                break;
            }
    ...
    @Override
    public void move() {
        System.out.println("Олень передвигается...");
        Cell currentCell = this.getCurrentcell();

        if (currentCell == null) {
            System.out.println("У оленя нет клетки!");
            return;
        }
        int maxSpeed = Data.DEER.getMaxSpeed();
        ...
        synchronized (currentCell) {
            synchronized (newCell) {
                if (currentCell.getAnimals().contains(this)) {
                    ...
                    System.out.println("Deer moved to (" + newX + ", " + newY + ")");
                }
            }
        }
    }
    ...
    public void die() {
        if (currentCell != null) {
            currentCell.removeAnimal(this);
            System.out.println("Deer died at (" + currentCell.getCoordinate().getX()
    ...
    public Optional<Livable> getOffspring() {
        System.out.println("Олень пытается размножится...");
        long count = currentCell.getAnimals().stream().filter(animal -> animal.getClass().equals(this.getClass())).count();

        if (count >= 2 && count < Data.DEER.getMaxQuantity()) {
            System.out.println("Олень размножился...");
            return Optional.of(new Deer());
        }
